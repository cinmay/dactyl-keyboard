...
def single_plate(cylinder_segments=100, side="right"):
    ...
    hotswap_soldertag_hight = 2.7
    hotswap_soldertag_lengtg = ((13.15 - 9.95) / 2) + 2.0
    ...
    hotswap_box = union([
        ...
        translate(
            box(hotswap_soldertag_hight, hotswap_soldertag_lengtg, hotswap_thickness + 1),
            (hotswap_length + (hotswap_soldertag_hight / 2), - hotswap_holes_offset / 2, 0)
        ),
        translate(
            box(hotswap_length * 1.5 + 0.1, hotswap_hight + 0.1, hotswap_thickness + 0.2),
            (-(hotswap_length / 2) + ((hotswap_length * 0.5) / 2), hotswap_holes_offset / 2, 0)
    ...
    kailh_holes = [
        ...
        translate(
            cylinder(hotswap_hole_diameter / 2, kailh_v2_switch_plate_thickness + 0.2),
            (hotswap_hole_1_x_offset, hotswap_hole_1_y_offset,
             (kailh_v2_switch_plate_thickness / 2) + 0)
        ),
        # translate(
        ...
        # ),
        translate(
            cylinder(hotswap_hole_diameter / 2, kailh_v2_switch_plate_thickness * 2),
            (hotswap_hole_2_x_offset, hotswap_hole_2_y_offset, kailh_v2_switch_plate_thickness / 2)
        ),
        # translate(
        #    box(hotswap_length, hotswap_hight, hotsawp_thickness + 0.2),
        ...
        translate(
            ...
            (- hotswap_y_offset, (hotswap_hole_1_y_offset + hotswap_hole_2_y_offset) / 2,
             - (kailh_v2_switch_plate_thickness / 2) + hotswap_hole_height + 1)
        ),
    ]
...
def trackball_cutout(segments=100, side="right"):
    if trackball_modular:
        hole_diameter = ball_diameter + 2 * (
                    ball_gap + ball_wall_thickness + trackball_modular_clearance + trackball_modular_lip_width) - .1
...
def default_thumb_1x_layout(shape, cap=False):
    ...
    if cap:
        ...
        if default_1U_cluster:
            shape_list.append(
                default_thumb_tr_place(rotate(rotate(shape, (0, 0, 90)), [0, 0, thumb_plate_tr_rotation])))
            shape_list.append(
                default_thumb_tr_place(rotate(rotate(shape, (0, 0, 90)), [0, 0, thumb_plate_tr_rotation])))
...
def minidox_thumb_post_tr():
    ...
    return translate(web_post(),
                     [(mount_width / 2) - post_adj,
                      ((mount_height / 2) + adjustable_plate_size(minidox_Usize)) - post_adj, 0]
                     )
...
def default_thumb_walls(skeleton=False):
    ...
    if default_1U_cluster:
        shape = union(
            [wall_brace(default_thumb_mr_place, 0, -1, web_post_br(), default_thumb_tr_place, 0, -1, web_post_br())])
    else:
        shape = union(
            [wall_brace(default_thumb_mr_place, 0, -1, web_post_br(), default_thumb_tr_place, 0, -1, thumb_post_br())])
    shape = union(
        [shape, wall_brace(default_thumb_mr_place, 0, -1, web_post_br(), default_thumb_mr_place, 0, -1, web_post_bl())])
    shape = union(
        [shape, wall_brace(default_thumb_br_place, 0, -1, web_post_br(), default_thumb_br_place, 0, -1, web_post_bl())])
    shape = union([shape, wall_brace(default_thumb_ml_place, -0.3, 1, web_post_tr(), default_thumb_ml_place, 0, 1,
                                     web_post_tl())])
    shape = union(
        [shape, wall_brace(default_thumb_bl_place, 0, 1, web_post_tr(), default_thumb_bl_place, 0, 1, web_post_tl())])
    shape = union(
        [shape, wall_brace(default_thumb_br_place, -1, 0, web_post_tl(), default_thumb_br_place, -1, 0, web_post_bl())])
    shape = union(
        [shape, wall_brace(default_thumb_bl_place, -1, 0, web_post_tl(), default_thumb_bl_place, -1, 0, web_post_bl())])
    # thumb, corners
    shape = union(
        [shape, wall_brace(default_thumb_br_place, -1, 0, web_post_bl(), default_thumb_br_place, 0, -1, web_post_bl())])
    shape = union(
        [shape, wall_brace(default_thumb_bl_place, -1, 0, web_post_tl(), default_thumb_bl_place, 0, 1, web_post_tl())])
    # thumb, tweeners
    shape = union(
        [shape, wall_brace(default_thumb_mr_place, 0, -1, web_post_bl(), default_thumb_br_place, 0, -1, web_post_br())])
    shape = union(
        [shape, wall_brace(default_thumb_ml_place, 0, 1, web_post_tl(), default_thumb_bl_place, 0, 1, web_post_tr())])
    shape = union(
        [shape, wall_brace(default_thumb_bl_place, -1, 0, web_post_bl(), default_thumb_br_place, -1, 0, web_post_tl())])
    if default_1U_cluster:
        shape = union([shape,
                       wall_brace(default_thumb_tr_place, 0, -1, web_post_br(), (lambda sh: key_place(sh, 3, lastrow)),
                                  0, -1, web_post_bl())])
    else:
        shape = union([shape, wall_brace(default_thumb_tr_place, 0, -1, thumb_post_br(),
...
def mini_thumb_walls(skeleton=False):
    # thumb, walls
    shape = union(
        [wall_brace(mini_thumb_mr_place, 0, -1, web_post_br(), mini_thumb_tr_place, 0, -1, mini_thumb_post_br())])
    shape = union(
    ...
    shape = union([shape,
                   wall_brace(mini_thumb_tr_place, 0, -1, mini_thumb_post_br(), (lambda sh: key_place(sh, 3, lastrow)),
                              0, -1, web_post_bl())])
...
def minidox_thumb_walls(skeleton=False):
    # thumb, walls
    shape = union([wall_brace(minidox_thumb_tr_place, 0, -1, minidox_thumb_post_br(), minidox_thumb_tr_place, 0, -1,
                              minidox_thumb_post_bl())])
    shape = union([shape,
                   wall_brace(minidox_thumb_tr_place, 0, -1, minidox_thumb_post_bl(), minidox_thumb_tl_place, 0, -1,
                              minidox_thumb_post_br())])
    shape = union([shape,
                   wall_brace(minidox_thumb_tl_place, 0, -1, minidox_thumb_post_br(), minidox_thumb_tl_place, 0, -1,
                              minidox_thumb_post_bl())])
    shape = union([shape,
                   wall_brace(minidox_thumb_tl_place, 0, -1, minidox_thumb_post_bl(), minidox_thumb_ml_place, -1, -1,
                              minidox_thumb_post_br())])
    shape = union([shape,
                   wall_brace(minidox_thumb_ml_place, -1, -1, minidox_thumb_post_br(), minidox_thumb_ml_place, 0, -1,
                              minidox_thumb_post_bl())])
    shape = union([shape,
                   wall_brace(minidox_thumb_ml_place, 0, -1, minidox_thumb_post_bl(), minidox_thumb_ml_place, -1, 0,
                              minidox_thumb_post_bl())])
    # thumb, corners
    shape = union([shape,
                   wall_brace(minidox_thumb_ml_place, -1, 0, minidox_thumb_post_bl(), minidox_thumb_ml_place, -1, 0,
                              minidox_thumb_post_tl())])
    shape = union([shape,
                   wall_brace(minidox_thumb_ml_place, -1, 0, minidox_thumb_post_tl(), minidox_thumb_ml_place, 0, 1,
                              minidox_thumb_post_tl())])
    # thumb, tweeners
    shape = union([shape,
                   wall_brace(minidox_thumb_ml_place, 0, 1, minidox_thumb_post_tr(), minidox_thumb_ml_place, 0, 1,
                              minidox_thumb_post_tl())])
    shape = union([shape, wall_brace(minidox_thumb_tr_place, 0, -1, minidox_thumb_post_br(),
                                     (lambda sh: key_place(sh, 3, lastrow)), 0, -1, web_post_bl())])
...
def carbonfet_thumb_walls(skeleton=False):
    # thumb, walls
    shape = union(
        [wall_brace(carbonfet_thumb_mr_place, 0, -1, web_post_br(), carbonfet_thumb_tr_place, 0, -1, web_post_br())])
    shape = union([shape, wall_brace(carbonfet_thumb_mr_place, 0, -1, web_post_br(), carbonfet_thumb_mr_place, 0, -1.15,
                                     web_post_bl())])
    shape = union([shape, wall_brace(carbonfet_thumb_br_place, 0, -1, web_post_br(), carbonfet_thumb_br_place, 0, -1,
                                     web_post_bl())])
    shape = union([shape, wall_brace(carbonfet_thumb_bl_place, -.3, 1, thumb_post_tr(), carbonfet_thumb_bl_place, 0, 1,
                                     thumb_post_tl())])
    shape = union([shape, wall_brace(carbonfet_thumb_br_place, -1, 0, web_post_tl(), carbonfet_thumb_br_place, -1, 0,
                                     web_post_bl())])
    shape = union([shape, wall_brace(carbonfet_thumb_bl_place, -1, 0, thumb_post_tl(), carbonfet_thumb_bl_place, -1, 0,
                                     web_post_bl())])
    # thumb, corners
    shape = union([shape, wall_brace(carbonfet_thumb_br_place, -1, 0, web_post_bl(), carbonfet_thumb_br_place, 0, -1,
                                     web_post_bl())])
    shape = union([shape, wall_brace(carbonfet_thumb_bl_place, -1, 0, thumb_post_tl(), carbonfet_thumb_bl_place, 0, 1,
                                     thumb_post_tl())])
    # thumb, tweeners
    shape = union([shape, wall_brace(carbonfet_thumb_mr_place, 0, -1.15, web_post_bl(), carbonfet_thumb_br_place, 0, -1,
                                     web_post_br())])
    shape = union([shape, wall_brace(carbonfet_thumb_bl_place, -1, 0, web_post_bl(), carbonfet_thumb_br_place, -1, 0,
                                     web_post_tl())])
    shape = union([shape,
                   wall_brace(carbonfet_thumb_tr_place, 0, -1, web_post_br(), (lambda sh: key_place(sh, 3, lastrow)), 0,
                              -1, web_post_bl())])
    return shape
...
if oled_center_row is not None:
    ...
    oled_mount_location_xyz = (np.array(base_pt1) + np.array(base_pt2)) / 2. + np.array(
        ((-left_wall_x_offset / 2), 0, 0)) + np.array(oled_translation_offset)
...
def screw_insert_all_shapes(bottom_radius, top_radius, height, offset=0, side='right'):
    ...
    shape = (
        ...
        translate(screw_insert(0, cornerrow, bottom_radius, top_radius, height, side=side),
